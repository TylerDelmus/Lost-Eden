...
public sealed class CatAnimPlayer : MonoBehaviour
{
    ...
    public CatAnimRuntimeClip EnsureClip(int animId)
    {
        ...
        AnimCalibration.Entry calib = AnimCalibration.GetOrDefault(animId);
        CatAnimRuntimeClip clip = CatAnimRuntimeClip.Create(
            catAnim,
            animId,
            _bones.Length,
            calib.TrimStart,
    ...
    void ApplyPose()
    {
        ...
        for (int i = 0; i < _bones.Length; i++)
        {
            ...
            _clipA.Evaluate(i, _timeA, out Vector3? posA, out Quaternion? rotA);
            if (posA.HasValue)
                pos = posA.Value;
            if (rotA.HasValue)
                rot = rotA.Value;
            ...
            {
                _clipB.Evaluate(i, _timeB, out Vector3? posB, out Quaternion? rotB);
                if (posB.HasValue)
