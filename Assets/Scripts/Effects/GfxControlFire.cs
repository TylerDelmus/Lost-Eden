...
/// <summary>
/// typeCode 1004 (0x3ec), stock <c>_GfxControlFire_t</c> driving its <c>GfxVisualSprite2Type0</c>. The
/// mechanics are <see cref="FireSim"/>; this binds them to a locator and draws the result.
///
/// Field 0 bit 1 puts the sprites in the locator's frame: stock then spawns them through an identity
/// matrix along the unit z (<c>101063d9</c> / <c>1010649b</c>) and places the visual on the locator
/// (<c>10106306</c> / <c>101062d5</c>). Otherwise they are spawned through the locator's world matrix and
/// stay where they were emitted.
///
/// Each sprite is an upright camera-facing quad (<c>10025391</c>) of width × height, centred on the
/// sprite, coloured by its ARGB and textured with its cell. The visual is built additive (SrcAlpha, One),
/// no Z-write, no culling. Stock Fire has no light.
/// </summary>
public sealed class GfxControlFire : GfxControl
{
    readonly FireSim _sim;
    ...
    public FireSim Sim => _sim;

    public GfxControlFire(
    ...
    /// <summary>Stock slot 8 (<c>100dc179</c>): +0x10 = seconds.</summary>
    ...
    void Body(float dt)
    {
        ...
        {
            // 100dbefc: a lost locator lets the sprites in flight finish.
            _sim.TerminateGracefully(Age);
            world = _visual;
        }

        // ProcessSprites gets the handler's GetWind (100dc0e2).
