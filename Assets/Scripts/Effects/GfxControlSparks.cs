...
/// <summary>
/// typeCode 1018 (0x3fa), stock <c>_GfxControlSparks_t</c> driving its <c>GfxVisualSprite2Type0</c>. The
/// mechanics are <see cref="SparksSim"/>; this binds them to a locator and draws the result.
///
/// Field 0 bit 1 puts the sprites in the locator's frame: stock then spawns them at zero along the unit
/// axes (<c>1010640a</c> / <c>10106439</c> / <c>1010646a</c> / <c>1010649b</c>) and places the visual on
/// the locator (<c>10106306</c> / <c>101062d5</c>). Otherwise they are spawned at the locator along its
/// axes in world space and stay where they were emitted.
///
/// Each sprite is an upright camera-facing quad (<c>10025391</c>): width along the camera's right, height
/// along its up, centred on the sprite, coloured by its ARGB and textured with its cell. The visual is
/// built with SrcAlpha and One, or InvSrcAlpha under flag 0x800, no Z-write, no culling. Stock Sparks
/// has no light.
/// </summary>
public sealed class GfxControlSparks : GfxControl
{
    readonly SparksSim _sim;
    ...
    /// <summary>Stock slot 8 (<c>100f1495</c>): +0x10 = seconds.</summary>
    ...
    void Body(float dt)
    {
        ...
        {
            // 100f24ec: a lost locator lets the sprites in flight finish.
            _sim.TerminateGracefully(Age);
            world = _visual;
        }

        // ProcessSprites gets the handler's GetWind (100f2550).
        ...
        else
        {
            ...
            ready = _sim.Step(Age, dt, o.x, o.y, o.z, _axes, wind.x, wind.y, wind.z);
        }
